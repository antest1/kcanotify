...
public class KcaMapHpPopupService extends Service {
                ...
                if (data != null) {
                    ...
                    for (int i = 0; i < data.size(); i++) {
                        ...
                        if (item.has("api_eventmap")) {
                            if (item.has("api_state") && item.get("api_state").getAsInt() != 2) {
                                int gauge_type = item.get("api_gauge_type").getAsInt();
                                int now_maphp = item.get("api_now_maphp").getAsInt();
                                int max_maphp = item.get("api_max_maphp").getAsInt();
    ...
    private String getMapHpStr(int type, int id, int current, int total) {
        if (type == 3) {
            return String.format(maphp_format, getMapString(id), "TP", current, total);
        } else {
            return String.format(maphp_format, getMapString(id), "HP", current, total);
        }
    }
